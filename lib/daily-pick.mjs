...
export function getDailyItems(items, count = 1, offset = 0, date = new Date()) {
  ...
  const itemCount = Math.min(Math.floor(count), items.length)
  const sequenceStart = (dayNumber + offset) * itemCount

  return Array.from({ length: itemCount }, (_, index) => {
    const rotationIndex = (start + (sequenceStart + index) * step) % items.length
    return items[rotationIndex]
  })
}

export function getDailyItem(items, offset = 0, date = new Date()) {
  const [item] = getDailyItems(items, 1, offset, date)

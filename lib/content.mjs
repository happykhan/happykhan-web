...
// Load and parse the conference presentations YAML file
export async function getConferencePresentationsData() {
  const filePath = path.join(process.cwd(), 'content', 'conference_presentations.yaml')
  try {
...
// Load and parse the CV YAML file
export async function getCVYamlData() {
  const filePath = path.join(process.cwd(), 'content', 'cv.yaml')
  const raw = await fs.readFile(filePath, 'utf8')
  const data = yaml.load(raw)
  // If wrapped in 'cv' key, return that, else return all
  return data.cv || data
}
...
export async function listMicrobinfie() {
  ...
  const items = await Promise.all(
    mdxFiles.map(async (file) => {
      ...
      if (!excerpt && content) {
        // Remove frontmatter markers and get first paragraph
        const cleaned = content.trim()
        // Find first paragraph (text before first double newline or heading)

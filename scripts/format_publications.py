...
cuttoff_year_string = f'{cutoff_year} and prior'
all_papers = {cuttoff_year_string: []} 
for record in csv.DictReader(open('content/tables/publications.csv', encoding='utf-8'), dialect=csv.excel_tab):
    if record['skip'].upper() != 'YES':
        if record['preprint'].lower() == 'yes' or record['Publication'] in ['bioRxiv', 'medRxiv', 'preprints', ]:
            ...
            try:
                if int(record['Year']) <= cutoff_year:
                    all_papers[cuttoff_year_string].append(record)
                else:
...
output_file = open('content/pages/publications.mdx', 'w', encoding='utf-8')
output_file.write('---\n')
